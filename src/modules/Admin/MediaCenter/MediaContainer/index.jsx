...
import { useAdminMedia, useUploadMediaMutation, useDeleteMediaMutation } from "@/lib/hooks/admin/useMedia";
...
export default function MediaContainer() {
    ...
    ] : (mediaQuery.data?.data?.media || mediaQuery.data?.data || []);

    const loading = mediaQuery.isPending;

    // Live — refresh when media is uploaded from another session
    useSocket({ "media:uploaded": mediaQuery.refetch, "media:deleted": mediaQuery.refetch });
    ...
    const handleUpload = async (e) => {
        ...
        try {
            ...
            await uploadMutation.upload(form);
            mediaQuery.refetch();
