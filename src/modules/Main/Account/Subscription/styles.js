...
export function printInvoice(inv) {
  const fmtINR = (n) => `₹${Number(n || 0).toLocaleString(DEFAULT_LOCALE)}`;
  const fmtDate = (d) => d ? new Date(d).toLocaleDateString(DEFAULT_LOCALE, { day: 'numeric', month: 'short', year: 'numeric' }) : '—';
