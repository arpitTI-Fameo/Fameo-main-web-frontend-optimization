...
export default function CommunityFeedback({ onOpenProfile, onOpenFeedback, showToast: toast }) {
  ...
  const { data: submissionsRes, isLoading: submissionsLoading } = useSubmissions(activeFilter);
  const { data: mentorsRes } = useMentorsOnline();
  const { mutateAsync: submitReviewMutation } = useSubmitReviewMutation();

  useEffect(() => {
    setLoading(submissionsLoading);
    if (submissionsRes?.data || submissionsRes) {
      const a = Array.isArray(submissionsRes?.data) ? submissionsRes.data : submissionsRes;
  ...
  }, [submissionsRes, activeFilter, submissionsLoading]);

  useEffect(() => {
    if (mentorsRes?.data || mentorsRes) {
      const a = Array.isArray(mentorsRes?.data) ? mentorsRes.data : mentorsRes;
      if (Array.isArray(a) && a.length > 0) {
        setMentorsOnline(a);
      }
    } else {
  ...
  async function submitReview(subId) {
    ...
    try {
      await submitReviewMutation({ subId, data: { checklist: cl, comment } });
