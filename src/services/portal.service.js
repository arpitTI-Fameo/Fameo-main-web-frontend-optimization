// The referral API lives on the UAT backend, NOT the main app backend that
// lib/apiClient.js targets. So this service calls uat-api.fameo.info directly
// with the app token (same pattern as account/profile/page.js), rather than
...
// Minimal fetch client for the UAT backend. Mirrors apiClient's response
...
async function uatFetch(path, { method = 'GET', body } = {}) {
  ...
  const res = await fetch(`${REFERRAL_API}${path}`, {
    method,
    headers: {
  ...
  const expired =
    ...
    /expired|invalid token/i.test(json?.message || '');
  if (expired) {
