...
// // All 4 community modals: Post, SubmitFeedback, Report, Onboarding
// import { useState, useRef } from 'react';
// import { SPACES_DATA, NICHE_OPTIONS, COMMUNITY_RULES, REPORT_REASONS } from '@/constants/community';
...
//         <label className="cm-field-label">{label}{required&&<span style={{ color:'var(--cm-red)' }}> *</span>}</label>
...
//   const TOPICS = ['Question','Tips & Advice','Case Study','Collaboration','Rant','Win 🎉','Feedback Needed'];

//   async function submit() {
//     if (!title.trim() || !content.trim()) { showToast?.('⚠️ Please fill title and content', 'warn'); return; }
//     setPosting(true);
//     const tagArr = tags.split(',').map(t=>t.trim()).filter(Boolean).slice(0,5);
//     try {
//       await svc.createPost({ spaceId:space||'general', topic, title, content, tags:tagArr });
//       onSuccess?.('✦ Discussion posted successfully!');
//     } catch(e) {
...
//             <input className="cm-input" value={title} onChange={e=>setTitle(e.target.value)} placeholder="What's your discussion about?" maxLength={200} />
//           </Field>

//           {/* Content */}
//           <Field label="Content" required hint={`${content.length} / 2000`}>
//             <textarea className="cm-textarea" rows={5} value={content} onChange={e=>setContent(e.target.value.slice(0,2000))}
//               placeholder="Share your thoughts, question, or insight…" />
//           </Field>

//           {/* Tags */}
//           <Field label="Tags" hint="Comma-separated, max 5">
//             <input className="cm-input" value={tags} onChange={e=>setTags(e.target.value)} placeholder="branddeals, instagram, growth" />
//           </Field>

//           {/* Rules */}
//           <div style={{ background:'var(--cm-bg3)', borderRadius:8, padding:'10px 12px', fontSize:12, color:'var(--cm-text3)', lineHeight:1.7 }}>
//             📋 <strong style={{ color:'var(--cm-text2)' }}>Posting rules:</strong> Be respectful · Stay on topic · No spam · Credit sources · Flag off-topic replies
//           </div>
//         </div>
//         <div className="cm-modal-footer">
//           <button className="cm-btn cm-btn-ghost" onClick={onClose}>Cancel</button>
//           <button className="cm-btn cm-btn-primary" onClick={submit} disabled={posting||!title.trim()||!content.trim()}>
...
//     { id:'brand',     icon:'🏷️', label:'Brand Pitch'   },
//   ];

//   async function submit() {
//     if (!consented) { showToast?.('⚠️ Please confirm the content declaration.', 'warn'); return; }
//     if (!objective.trim()) { showToast?.('⚠️ Please describe your objective.', 'warn'); return; }
//     setSubmitting(true);
//     try {
...
//                   style={{ padding:'12px', background: contentType===t.id?'var(--cm-accent-soft)':'var(--cm-bg3)', border:`1px solid ${contentType===t.id?'var(--cm-accent)':'var(--cm-border2)'}`, borderRadius:10, cursor:'pointer', textAlign:'center', transition:'all .14s' }}>
...
//           {/* Upload */}
//           <Field label="Upload Content">
//             <div className="cm-upload-zone" onClick={() => fileRef.current?.click()}>
//               <input ref={fileRef} type="file" accept="video/*,image/*,audio/*,.pdf" style={{ display:'none' }}
...
//                   <div style={{ fontSize:11.5, color:'var(--cm-text3)', marginTop:4 }}>Max 500MB · No nudity or adult content</div>
//                 </>
//               )}
//             </div>
//           </Field>
...
//               placeholder="What specific feedback are you looking for? E.g. 'Is my hook strong enough for the first 3 seconds?'" />
//           </Field>

//           {/* Tags */}
//           <Field label="Tags" hint="Max 5">
//             <input className="cm-input" value={tags} onChange={e=>setTags(e.target.value)} placeholder="reel, instagram, hook" />
//           </Field>

//           {/* Consent */}
//           <div style={{ background:'rgba(124,58,237,.08)', border:'1px solid rgba(124,58,237,.2)', borderRadius:10, padding:14, marginBottom:8 }}>
//             <div style={{ display:'flex', gap:12, alignItems:'flex-start', cursor:'pointer' }} onClick={() => setConsented(v=>!v)}>
//               <div className={`cm-consent-check${consented?' checked':''}`}>{consented&&'✓'}</div>
//               <div style={{ fontSize:12.5, color:'var(--cm-text2)', lineHeight:1.6 }}>
//                 I confirm this content is <strong style={{ color:'var(--cm-text)' }}>my original work</strong>, does not contain copyrighted material without permission, and complies with community guidelines. I understand my submission will be visible to peers for review.
//               </div>
//             </div>
//           </div>
//         </div>
//         <div className="cm-modal-footer">
//           <button className="cm-btn cm-btn-ghost" onClick={onClose}>Cancel</button>
//           <button className="cm-btn cm-btn-primary" id="submit-btn"
//             style={{ opacity: consented?1:.5, pointerEvents: consented?'auto':'none' }}
//             onClick={submit} disabled={submitting||!consented}>
...
//   { id:'harassment',          label:'Harassment or threatening behaviour', sla:'Priority review within 2 hours' },
//   { id:'adult',               label:'Adult or inappropriate content' },
//   { id:'misinfo',             label:'Misinformation or false claims' },
//   { id:'vulgarity',           label:'Excessive vulgarity or offensive language' },
//   { id:'undisclosed_promotion', label:'Undisclosed paid promotion' },
//   { id:'copyright',           label:'Copyright or IP violation' },
...
//     setSubmitting(true);
//     try {
//       await svc.reportContent({ targetType: target?.type||'post', targetId: target?.id, reason, description: details });
//       setSubmitted(true);
//     } catch {
...
//   if (submitted) return (
//     <Overlay onClose={onClose}>
//       <div className="cm-modal-box" style={{ maxWidth:420 }}>
//         <div style={{ padding:'40px 28px', textAlign:'center' }}>
//           <div style={{ fontSize:40, marginBottom:16 }}>✅</div>
//           <div style={{ fontFamily:'var(--cm-font-display)', fontSize:20, fontWeight:700, color:'var(--cm-text)', marginBottom:8 }}>Report Submitted</div>
...
//             {reason==='harassment' && <><br /><strong style={{ color:'var(--cm-orange)' }}>Priority case — reviewing within 2 hours.</strong></>}
//           </div>
...
//               Reporting: <strong style={{ color:'var(--cm-text)' }}>{target.title}</strong>
//             </div>
//           )}
...
//                 <div style={{ flex:1 }}>
//                   {r.label}
//                   {r.sla && reason===r.id && <div style={{ fontSize:11, color:'var(--cm-orange)', marginTop:2, fontWeight:600 }}>⚡ {r.sla}</div>}
//                 </div>
//               </div>
//             ))}
//           </Field>
...
//             <textarea className="cm-textarea" rows={3} value={details} onChange={e=>setDetails(e.target.value.slice(0,500))}
//               placeholder="Optional: provide more context to help our team review faster…" />
//           </Field>
//         </div>
//         <div className="cm-modal-footer">
//           <button className="cm-btn cm-btn-ghost" onClick={onClose}>Cancel</button>
//           <button className="cm-btn cm-btn-primary" onClick={submit} disabled={submitting||!reason}>
...
//   const RULES_LIST = [
//     'Respect fellow creators — constructive feedback only',
//     'No spam or unsolicited promotion without context',
//     'Share original content and credit collaborators',
//     'Help each other grow — community over competition',
//     'Report violations using the ⋯ menu on any post',
//   ];
...
//   return (
//     <div className="cm-onboard-overlay" style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.85)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center' }}>
//       <div className="cm-onboard-box" style={{ width:520, maxWidth:'calc(100vw - 32px)', background:'var(--cm-bg2)', border:'1px solid var(--cm-border2)', borderRadius:'var(--cm-radius-xl)', overflow:'hidden', boxShadow:'var(--cm-shadow-lg)' }}>

//         {/* Progress pills */}
//         <div style={{ padding:'20px 28px 0', textAlign:'center' }}>
//           <div style={{ display:'flex', justifyContent:'center', gap:6, marginBottom:20 }}>
//             {[1,2,3].map(s => (
//               <div key={s} style={{ width:28, height:4, borderRadius:2, background: s<=step?'var(--cm-accent)':'var(--cm-border2)', transition:'background .2s' }} />
//             ))}
//           </div>
//         </div>
...
//               <div style={{ fontSize:13.5, color:'var(--cm-text3)', lineHeight:1.6, marginBottom:24 }}>
//                 Connect with 10,000+ creators, get feedback on your content, and learn from industry experts.
//               </div>
...
//               <button className="cm-btn cm-btn-primary" style={{ height:40 }} onClick={() => setStep(2)}>Get Started →</button>
//             </div>
//           </div>
//         )}
...
//                 <div style={{ fontFamily:'var(--cm-font-display)', fontSize:20, fontWeight:700, color:'var(--cm-text)', marginBottom:6 }}>Pick Your Niches</div>
//                 <div style={{ fontSize:13, color:'var(--cm-text3)' }}>Select up to 2 content categories</div>
//               </div>
//               {niches.length >= 2 && (
//                 <div style={{ fontSize:11.5, color:'var(--cm-orange)', marginBottom:10, textAlign:'center' }}>⚠️ Maximum 2 niches selected. Deselect one to change.</div>
//               )}
//               <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
//                 {NICHE_OPTIONS.map(n => (
//                   <div key={n.id} className={`cm-niche-option${niches.includes(n.id)?' sel':''}`} onClick={() => toggleNiche(n.id)}>
//                     {n.label}
...
//               <button className="cm-btn cm-btn-primary" style={{ height:40 }} onClick={() => setStep(3)} disabled={niches.length === 0}>
//                 Continue →
//               </button>
//             </div>
//           </div>
...
//                 <div style={{ fontSize:13, color:'var(--cm-text3)' }}>Please review and agree to our community guidelines</div>
//               </div>
...
//                     <span style={{ color:'var(--cm-accent)', fontWeight:700, flexShrink:0 }}>{i+1}.</span>{r}
//                   </div>
//                 ))}
//               </div>
//               <div style={{ background:'rgba(124,58,237,.08)', border:'1px solid rgba(124,58,237,.2)', borderRadius:10, padding:14 }}>
//                 <div style={{ display:'flex', gap:12, alignItems:'flex-start', cursor:'pointer' }} onClick={() => setAgreeTos(v=>!v)}>
//                   <div className={`cm-consent-check${agreeTos?' checked':''}`} style={{ marginTop:2 }}>{agreeTos&&'✓'}</div>
//                   <div style={{ fontSize:12.5, color:'var(--cm-text2)', lineHeight:1.6 }}>
...
//               <button className="cm-btn cm-btn-primary" style={{ height:40, opacity:agreeTos?1:.5, pointerEvents:agreeTos?'auto':'none' }}
//                 onClick={onComplete} disabled={!agreeTos}>
//                 Enter Community ✦
//               </button>
//             </div>
//           </div>
...
export function PostModal({ onClose, onSuccess, showToast }) {
  ...
  async function submit() {
    ...
    const tagArr = tags.split(',').map(t => t.trim()).filter(Boolean).slice(0,5);
    try {
      await svc.createPost({ spaceId:space, topic, title:title.trim(), content:content.trim(), tags:tagArr });
      onSuccess?.('✦ Discussion posted successfully!');
    } catch(e) {
      showToast?.(e.message || 'Failed to post', 'error');
      setPosting(false);
