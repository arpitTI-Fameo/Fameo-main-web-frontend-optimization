// app/api/smartauth/profile/route.js
// Authenticated proxy to the SmartAuth KYC profile lookup endpoint.
//
...
const ALLOWED = ['requestId', 'referenceId', 'consent'];

export async function POST(req) {
  const { user, error } = await requireUser(req);
  if (error) return error;

  const { ok, retryAfter } = rateLimit(`smartauth:profile:${user.id}`, {
    limit: 10,
    windowMs: 60_000,
  });
  ...
  if (!process.env.SMARTAUTH_KEY) {
    console.error('[smartauth/profile] SMARTAUTH_KEY is not configured');
  ...
  try {
    const res = await fetch('https://prod.smartauth.co/C9S1', {
      method: 'POST',
      headers: {
    ...
    console.error('[smartauth/profile] upstream call failed:', err?.message);
    return NextResponse.json(
