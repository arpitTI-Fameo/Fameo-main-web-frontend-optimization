...
const ALLOWED = ['pan', 'panNumber', 'name', 'dob', 'consent'];

export async function POST(req) {
  const { user, error } = await requireUser(req);
  if (error) return error;

  const { ok, retryAfter } = rateLimit(`smartauth:pan:${user.id}`, {
    limit: 5,
    windowMs: 60_000,
  });
  ...
  try {
    ...
    console.error('[smartauth/pan] upstream call failed:', err?.message);
    return NextResponse.json(
